...
* [`src/rf10x_bug/`](src/rf10x_bug/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/rf10x_bug/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `rf10x-bug` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/rf10x_bug/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns rf10x-bug.example
  (:require [rf10x-bug.config :as config])
